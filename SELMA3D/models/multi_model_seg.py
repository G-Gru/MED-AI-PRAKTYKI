...
if os.path.exists(model_cls_path):
    model_cls.load_state_dict(torch.load(model_cls_path, weights_only=True))
...
# =============================================================================
# FAZA 2: TRENING MODELI SEGMENTACYJNYCH (ROZDZIAŁ DYNAMICZNY)
# =============================================================================
log(f"\n[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] START FAZY 2: SEGMENTACJA KASKADOWA (2 MODELE)")

# Model 0: dla 'contiguous_structures' (klasa 0)
model_seg_0 = SegResNet(
    spatial_dims=3, in_channels=1, out_channels=NUM_CLASSES_SEG,
    init_filters=32, blocks_down=[1, 2, 2, 4], blocks_up=[1, 1, 1], dropout_prob=0.2,
).to(DEVICE)

# Model 1: dla 'isolated_structures' (klasa 1)
model_seg_1 = SegResNet(
    spatial_dims=3, in_channels=1, out_channels=NUM_CLASSES_SEG,
    init_filters=32, blocks_down=[1, 2, 2, 4], blocks_up=[1, 1, 1], dropout_prob=0.2,
).to(DEVICE)

model_seg_0_path = os.path.join(model_dir, "best_metric_model_seg0.pth")
model_seg_1_path = os.path.join(model_dir, "best_metric_model_seg1.pth")

if os.path.exists(model_seg_0_path) and os.path.exists(model_seg_1_path):
    model_seg_0.load_state_dict(torch.load(model_seg_0_path, weights_only=True))
    model_seg_1.load_state_dict(torch.load(model_seg_1_path, weights_only=True))
    log("Wczytano wagi obu segmentatorów. Kontynuacja treningu.")

max_epochs_seg = 1000
val_interval = 10

optimizer_seg_0 = torch.optim.AdamW(model_seg_0.parameters(), lr=2e-4, weight_decay=1e-4)
optimizer_seg_1 = torch.optim.AdamW(model_seg_1.parameters(), lr=2e-4, weight_decay=1e-4)

scheduler_seg_0 = CosineAnnealingWarmRestarts(optimizer_seg_0, T_0=50, T_mult=2, eta_min=1e-6)
scheduler_seg_1 = CosineAnnealingWarmRestarts(optimizer_seg_1, T_0=50, T_mult=2, eta_min=1e-6)
...
scaler_seg_0 = torch.amp.GradScaler('cuda')
scaler_seg_1 = torch.amp.GradScaler('cuda')

class DualCascadePredictor(nn.Module):
    """Dynamicznie kieruje patch do odpowiedniego modelu segmentacji na podstawie predykcji klasyfikatora."""
    def __init__(self, classifier, segmenter_0, segmenter_1):
        super().__init__()
        self.classifier = classifier
        self.segmenter_0 = segmenter_0
        self.segmenter_1 = segmenter_1
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            B, _, D, H, W = x.shape
            output = torch.zeros((B, NUM_CLASSES_SEG, D, H, W), device=x.device, dtype=x.dtype)
            
            # Maski do selekcji odpowiednich elementów z batcha
            mask_0 = pred_classes == 0
            mask_1 = pred_classes == 1
            
            # Wymagane rzutowanie wyjścia do typu zmiennoprzecinkowego (dtype=Float vs Half z autocoast)
            if mask_0.any():
                output[mask_0] = self.segmenter_0(x[mask_0]).to(output.dtype)
            if mask_1.any():
                output[mask_1] = self.segmenter_1(x[mask_1]).to(output.dtype)
...
for epoch in range(max_epochs_seg):
    ...
    for batch_data in train_loader:
        ...
        if mask_0.any():
            step_0 += 1
            optimizer_seg_0.zero_grad()
            with torch.amp.autocast('cuda'):
                outputs_0 = model_seg_0(inputs[mask_0])
                loss_0 = loss_function_seg(outputs_0, labels_seg[mask_0])
            
            scaler_seg_0.scale(loss_0).backward()
            scaler_seg_0.step(optimizer_seg_0)
            scaler_seg_0.update()
        ...
        if mask_1.any():
            step_1 += 1
            optimizer_seg_1.zero_grad()
            with torch.amp.autocast('cuda'):
                outputs_1 = model_seg_1(inputs[mask_1])
                loss_1 = loss_function_seg(outputs_1, labels_seg[mask_1])
            
            scaler_seg_1.scale(loss_1).backward()
            scaler_seg_1.step(optimizer_seg_1)
            scaler_seg_1.update()
            epoch_loss_1 += loss_1.item()

    avg_loss_0 = epoch_loss_0 / max(step_0, 1)
    avg_loss_1 = epoch_loss_1 / max(step_1, 1)
    
    scheduler_seg_0.step()
    scheduler_seg_1.step()
    
    if (epoch + 1) % 5 == 0:
        lr_0 = scheduler_seg_0.get_last_lr()[0]
        lr_1 = scheduler_seg_1.get_last_lr()[0]
        log(f"Faza 2 - Epoka {epoch + 1}/{max_epochs_seg} | Loss 0: {avg_loss_0:.4f} | Loss 1: {avg_loss_1:.4f} | LR 0: {lr_0:.6f} | LR 1: {lr_1:.6f}")

    # Ewaluacja (Sliding Window z dynamicznym routingiem)
    if (epoch + 1) % val_interval == 0:
        cascade_predictor = DualCascadePredictor(model_cls, model_seg_0, model_seg_1).eval()
        ...
        with torch.no_grad():
            ...
            if metric >= best_metric:
                ...
                torch.save(model_seg_0.state_dict(), model_seg_0_path)
                torch.save(model_seg_1.state_dict(), model_seg_1_path)
                log(f"> Zapisano nowe segmentatory (Dice: {best_metric:.4f})")
                epoch_no_improve = 0
            else:
